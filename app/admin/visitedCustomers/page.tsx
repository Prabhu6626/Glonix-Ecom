...
import { Eye, ArrowLeft, Mail, Building, Calendar, User as UserIcon } from "lucide-react"
...
function VisitedCustomersContent() {
  ...
  useEffect(() => {
    const visitedCustomers = AuthService.getUsersByFabricationStatus(1)
    setCustomers(visitedCustomers)
    setLoading(false)
  }, [])
      ...
      <div className="flex items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-2">
            Users who have checked fabrication prices ({customers.length} total)
          </p>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-orange-600">Total Visited</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          {customers.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">No Visited Customers</h3>
              <p className="text-slate-600">No customers have visited the fabrication page yet.</p>
            </div>
          ) : (
              ...
              {customers.map((customer) => (
                <div key={customer.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className="bg-orange-200 text-orange-800">Visited</Badge>
