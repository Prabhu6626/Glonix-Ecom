...
import { getAllProducts, addProduct, updateProduct, deleteProduct, initializeMockData } from "@/lib/admin-utils"
import type { Product, ProductFormData } from "@/lib/types"
import { Package, Search, Plus, Edit, Trash2, Filter, Star } from "lucide-react"
...
function ProductManagementContent() {
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadPromises = Array.from(files).map(async (file) => {
      ...
      formData.append('username', 'admin') // You can get this from auth context
  ...
  const handleSaveProduct = async () => {
    try {
      if (editingProduct) {
        ...
        // Add new product
        const newProduct = addProduct(productForm)
        if (newProduct) {
          loadProducts()
          setIsAddDialogOpen(false)
          resetForm()
